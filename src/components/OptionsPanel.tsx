...
const OptionsPanel: React.FC<OptionsPanelProps> = memo(
  ({ options, onChange }) => {
    ...
    const checkboxBaseClasses =
      'h-4.5 w-4.5 rounded border-neutral-300 dark:border-neutral-600 text-sky-600 focus:ring-sky-500 bg-white dark:bg-neutral-700 dark:checked:bg-sky-600 dark:checked:border-sky-600 shadow-sm transition-all cursor-pointer';
    ...
    const preambleOptions = [
      ...
      {
        value:
          'The following are the complete project code files for my application.',
        label: 'Complete project code',
      },
      {
        value:
          "Below is a comprehensive collection of the project's source files.",
        label: 'Comprehensive source files',
      },
      {
        value:
          'Here are the selected files from my project that I need help with.',
        label: 'Selected files for assistance',
      },
    ];

    const isCustomPreamble = !preambleOptions
      .slice(1)
      .some((opt) => opt.value === options.preambleText);
            ...
            <div className="pl-1 space-y-2.5">
              <div className="flex items-center">
                ...
                <label
                  ...
                  className={`ml-2.5 ${labelBaseClasses} flex items-center`}
                >
